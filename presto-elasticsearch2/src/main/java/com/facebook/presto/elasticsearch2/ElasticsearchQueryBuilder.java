...
import static com.facebook.presto.elasticsearch2.ElasticsearchErrorCode.ELASTIC_SEARCH_CLIENT_ERROR;
...
public class ElasticsearchQueryBuilder
{
    ...
    private final boolean hasNestedField;
    private final Optional<TupleDomain<List<String>>> nestedTupleDomain;

    public ElasticsearchQueryBuilder(List<ElasticsearchColumnHandle> columnHandles, ElasticsearchConnectorConfig config, ElasticsearchSplit split, boolean isNested)
    {
        ...
        try {
            ...
            client.addTransportAddress(new InetSocketTransportAddress(InetAddress.getByName(split.getSearchNode()), split.getPort()));
        }
        catch (IOException e) {
    ...
    public SearchRequestBuilder buildScrollSearchRequest()
    {
        ...
        if (!hasNestedField) {
            List<String> fields = columns.stream().map(c -> c.getColumnJsonPath()).collect(toList());
            searchRequestBuilder.addFields(fields.toArray(new String[fields.size()]));
        }
