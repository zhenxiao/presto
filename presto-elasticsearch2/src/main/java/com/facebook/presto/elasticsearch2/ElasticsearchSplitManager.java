    ...
    public ConnectorSplitSource getSplits(ConnectorTransactionHandle transactionHandle, ConnectorSession session, ConnectorTableLayoutHandle layout, SplitSchedulingStrategy splitSchedulingStrategy)
    {
        ...
        for (String index : indices) {
            ...
            for (ClusterSearchShardsGroup group : response.getGroups()) {
                ...
                ElasticsearchSplit split = new ElasticsearchSplit(connectorId, table, index, group.getShardId(), nodes[nodeIndex].getName(), nodes[nodeIndex].getAddress().getPort(), layoutHandle.getTupleDomain());
