...
public class TestUberSystemAccessControl
{
    ...
    public void testImpersonations()
    {
        ...
        accessControl.checkCanSetUser(janus.getPrincipal(), "alice");
        accessControl.checkCanSetUser(janus.getPrincipal(), "bob");
        accessControl.checkCanSetUser(janus.getPrincipal(), "cathy");

        // Presto should be able to impersonate all users
        accessControl.checkCanSetUser(presto.getPrincipal(), "alice");
        accessControl.checkCanSetUser(presto.getPrincipal(), "bob");
        accessControl.checkCanSetUser(presto.getPrincipal(), "cathy");

        // Alice shouldn't be able to impersonate any user
        assertDenied(() -> accessControl.checkCanSetUser(alice.getPrincipal(), "bob"));
        assertDenied(() -> accessControl.checkCanSetUser(alice.getPrincipal(), "cathy"));

        // Bob should be able to impersonate only alice
        accessControl.checkCanSetUser(bob.getPrincipal(), "alice");
        assertDenied(() -> accessControl.checkCanSetUser(bob.getPrincipal(), "cathy"));
    }
